const app = {
    ...
    showView(viewName) {
        if (viewName === 'teacher') {
            const pass = prompt('Unesite lozinku za nastavnika (podrazumevano: nastavnik123):');
    ...
    renderHome() {
                    ...
                    <div class="bg-slate-50 rounded-[2.5rem] p-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ${this.chapters.map((chapter, index) => `
                                <div class="bg-white rounded-2xl border border-slate-200 p-6 hover:border-blue-300 hover:shadow-lg transition-all">
                                    <div class="flex items-center gap-4 mb-4">
                                        <div class="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
                                            <i data-lucide="${chapter.icon || 'book'}" class="w-6 h-6 text-blue-600"></i>
                                        </div>
                                        <div>
                                            <h4 class="font-bold text-slate-900 text-xl">${chapter.title}</h4>
        ...
        grid.innerHTML = this.chapters.map((chapter, index) => {
            ...
            return `
                <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden group ${!isUnlocked ? 'opacity-75 grayscale' : ''} flex flex-col transform hover:-translate-y-2">
                    <div class="h-40 bg-slate-50 relative overflow-hidden">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center">
                            <i data-lucide="${chapter.icon || 'book'}" class="w-16 h-16 ${isUnlocked ? 'text-blue-600' : 'text-slate-300'} transition-transform group-hover:scale-110"></i>
                        </div>
                    </div>
                    <div class="p-10 flex-1 flex flex-col">
                        ...
                        <h3 class="text-2xl font-black text-slate-900 mb-4 group-hover:text-blue-600 transition-colors leading-tight">${chapter.title}</h3>
            ...
            return `
                <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden group ${!isUnlocked ? 'opacity-75 grayscale' : ''} flex flex-col transform hover:-translate-y-2">
                    <div class="p-10 flex-1 flex flex-col">
                        ...
                        <h3 class="text-2xl font-black text-slate-900 mb-4 group-hover:text-blue-600 transition-colors leading-tight">${lesson.title}</h3>
    ...
    approveStudent(studentId) {
        ...
        // Move to approved students (permanent storage)
        const approvedStudents = this.getApprovedStudents();
    ...
    revokeAccess(studentId) {
        ...
        // If student is currently logged in, log them out
